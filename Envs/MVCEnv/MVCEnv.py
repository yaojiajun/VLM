...
def solve_mvc_gurobi(vertices, edges):
    """
    Solve the Minimum Vertex Cover exactly using Gurobi.
    ...
    -------
    cover : list of int
        The set of vertices in an optimal minimum vertex cover (sorted).
    """
    m = Model("MVC")
    ...
    # 4) Collect vertices in the cover
    cover = []
    for v in vertices:
        if x[v].X > 0.5:
            cover.append(v)
...
###############################################################################
# Tagging Function for JSON
###############################################################################

def tag_prompt_and_transform_to_json_mvc_with_neighbors(num_nodes, edge_list, cover, k_neighbors=2):
    """
    Create a JSON-like dict describing:
      - The instruction to solve MVC
      - The 'input' description, which includes top-k adjacency info in format N0:[4,2,#7,#4]
      - The 'output' (cover set and objective)
    ...
    instruction = (
        ...
        f"and edges specified below. For each node, we also provide up to {k_neighbors} neighbors "
        "with the largest degrees. Find a minimum vertex cover: a smallest set of vertices "
        "such that every edge has at least one endpoint in this set.\n\n"
        ...
        "1. Set: The list of vertices in the minimum vertex cover.\n"
        "2. Objective: The size of that set."
    )

    output_str = (
        f"Set: {cover}, "
        f"Objective: {len(cover)}"
    )
...
###############################################################################
# Environment Class for Generating Random MVC Instances
# with random selection of either Erdős–Rényi or Barabási–Albert
###############################################################################

class MVCEnv:
    """
    Environment class to generate random graph instances for the Minimum Vertex Cover (MVC) problem.
    ...
    def generate_instances(self, n_instance: int):
        """
        Generate a list of random MVC instances: (num_nodes, edges, dist_type).
        """
        instances = []
    ...
    def generate_instances_and_save(
            ...
            n_instance: int,
            file_name_json: str,
            k_neighbors: int = 3,  # <--- new parameter for top-k
            save_pkl: bool = False,
            rl_data: bool = False
    ):
        valid_json_data = []
        ...
        pbar = tqdm(total=n_instance, desc="Generating MVC instances (ER or BA)")
        ...
        while count_valid < n_instance:
            ...
            try:
                cover = solve_mvc_gurobi(vertices, edges)
                # Tag into JSON
                mvc_json = tag_prompt_and_transform_to_json_mvc_with_neighbors(num_nodes, edges, cover)
                ...
                valid_json_data.append(mvc_json)
                valid_instances_pkl.append((num_nodes, edges, cover))
        ...
        if save_pkl:
            save_dataset(valid_instances_pkl, "./test_large.pkl")

        print(f"Generated and saved {len(valid_json_data)} MVC instances to {file_name_json}.")


def transform_pkl_to_json(pkl_file: str, json_file: str, k_neighbors: int = 3, rl_data: bool = False):
    """
    Read a PKL file containing MVC instances (as networkx Graph objects) and transform them to JSON format.
    
    Parameters
    ----------
    pkl_file : str
        Path to the input PKL file containing networkx Graph objects.
    json_file : str
        Path to save the output JSON file.
    k_neighbors : int
        Number of neighbors to include in the instance description.
    rl_data : bool
        Whether to include the raw instance data in the JSON output.
    """
    # Load the PKL file
    ...
    print(f"Processing {len(instances)} instances from {pkl_file}...")
    for graph in tqdm(instances, desc="Converting instances to JSON"):
        try:
            ...
            # Solve MVC for this instance
            cover = solve_mvc_gurobi(vertices, edges)
            
            # Tag into JSON format
            mvc_json = tag_prompt_and_transform_to_json_mvc_with_neighbors(
                num_nodes, edges, cover, k_neighbors
            )
            ...
            print(f"Warning: Failed to process an instance: {e}")
            continue
    ...
    print(f"Successfully converted {len(valid_json_data)} instances to {json_file}")
...
if __name__ == "__main__":
    # Example 1: Generate new instances
    # mvc_env = MVCEnv(
    #     n_node_range=[70, 100],
    ...
    # mvc_env.generate_instances_and_save(
    #     n_instance=100,
    #     file_name_json="test_large.json",
    #     save_pkl=True,
    #     rl_data=True
    # )

    # Example 2: Transform existing PKL file to JSON
    transform_pkl_to_json(
        pkl_file="E:\\program\\fm4co\\Envs\\MVCEnv\\mvc.pkl",
        json_file="data.json",
        k_neighbors=2,
        rl_data=True
    )
