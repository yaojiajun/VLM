"""
Generate images_grid_uniform/ — same as images_grid/ but all customer nodes
have the SAME marker size (uniform, not proportional to demand).

Source: data_sft/cvrp/train_cvrp-001.json  (first 50000 samples)
Output: data_sft/cvrp/images_grid_uniform/  instance_{idx:06d}_n{num_nodes}.png
"""

import json, re, math, os, argparse
import matplotlib
matplotlib.use('Agg')
...
def draw_uniform_grid(nodes, num_nodes, capacity, inst_idx, out_path):
    """Grid-style image with uniform node size (demand shown in label only)."""
    ...
    for nid, (col, row) in positions.items():
        ...
        if nid == 0:
            ...
            # Uniform size — all customers the same size
            size = base_ms * 1.0
...
def convert_single(args_tuple):
    ...
    out_path  = os.path.join(output_dir, f"instance_{idx + 1:06d}_n{num_nodes}.png")
...
def main():
    ...
    parser.add_argument('--output_dir',  type=str, default='./data_sft/cvrp/images_grid_uniform')
    parser.add_argument('--num_samples', type=int, default=50000)
    parser.add_argument('--num_workers', type=int, default=12)
    args = parser.parse_args()

    json_path = os.path.join(args.data_dir, args.train_json)
    os.makedirs(args.output_dir, exist_ok=True)

    with open(json_path) as f:
        data = json.load(f)
    data = data[:args.num_samples]
    print(f"Generating {len(data)} uniform-grid images -> {args.output_dir}")

    tasks = [(i, sample, args.output_dir) for i, sample in enumerate(data)]
    with Pool(processes=args.num_workers) as pool:
        for i, _ in enumerate(tqdm(pool.imap_unordered(convert_single, tasks, chunksize=20),
                                   total=len(tasks))):
            pass
    print("Done.")
