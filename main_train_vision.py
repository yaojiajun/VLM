...
# ─────────────────────────────────────────────────────────────
# Prompt template
# A short instruction is appended to the image.
# The text input (node list) is intentionally omitted because
# the grid image already encodes coordinates and demands.
# ─────────────────────────────────────────────────────────────
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Vision SFT trainer for CVRP (Qwen2.5-VL)')
    ...
    parser.add_argument('--data_dir', type=str,
                        ...
                        help='Directory containing train JSON, eval JSON and images_grid/')
    parser.add_argument('--train_json', type=str, default='train_cvrp-001.json')
    parser.add_argument('--eval_json', type=str, default='eval/test.json')
    parser.add_argument('--images_grid_dir', type=str, default='images_grid',
                        help='Sub-directory (under data_dir) that holds images_grid PNGs')
...
# naming: instance_{idx:06d}_n{num_nodes}.png  (1-based index)
# ─────────────────────────────────────────────────────────────
def get_image_path(images_dir: str, record_idx: int, num_nodes: str) -> str:
    """Return the expected image path for the i-th record (0-based)."""
    filename = f"instance_{record_idx + 1:06d}_n{num_nodes}.png"
...
def load_vision_dataset(json_path: str, images_dir: str, max_samples: int) -> Dataset:
    ...
    for idx, rec in enumerate(records):
        img_path = get_image_path(images_dir, idx, rec["num_nodes"])
...
def train_model(args):
    ...
    if args.output_dir is None:
        dir_out = (
            f"output_vision_alpha{args.lora_alpha}_r{args.lora_r}"
    ...
    # ── Dataset ─────────────────────────────────────────────
    images_dir = os.path.join(args.data_dir, args.images_grid_dir)
    train_json_path = os.path.join(args.data_dir, args.train_json)
    eval_json_path = os.path.join(args.data_dir, args.eval_json)

    print("Loading training dataset...")
    train_dataset = load_vision_dataset(train_json_path, images_dir, args.num_train_samples)
    train_dataset = train_dataset.shuffle(seed=args.seed)

    # Eval dataset: no images_grid for eval, so we load text-only as a
    # best-effort reference. Pass eval_dataset=None to skip SFTTrainer eval.
    eval_dataset = None

    # ── Data collator ────────────────────────────────────────
    # Images are loaded lazily here (one batch at a time) via make_conversation.
