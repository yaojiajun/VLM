...
def feasibility_reward_func_cvrp(completions, instance_coords, instance_demands, instance_capacity, **kwargs):
    ...
    Weights:
      parse           0.1  — solution can be parsed
      depot_constraint 0.1 — every route starts and ends at depot (node 0)
      capacity        0.35 — no route exceeds vehicle capacity
      coverage        0.35 — all customers visited (partial credit by ratio)
      no_duplicate    0.10 — no customer visited more than once
    """
    scores = []
    ...
    for i, response in enumerate(completions):
        ...
        # Depot constraint: every route must start and end at node 0
        depot_ok = all(route and route[0] == 0 and route[-1] == 0 for route in predicted_routes)
        if depot_ok:
            score += weights["depot_constraint"]

        # Capacity constraint: partial credit proportional to routes that satisfy capacity
        try:
            n_routes      = len(predicted_routes)
            routes_ok     = sum(
                1 for route in predicted_routes
                if sum(demands[n] for n in route if n != 0) <= capacity
            )
            score += weights["capacity"] * (routes_ok / max(1, n_routes))
        except Exception:
            pass
        ...
        try:
            ...
            for route in predicted_routes:
                all_visited.extend(n for n in route if n != 0)

            visited_set       = set(all_visited)
            covered           = visited_set.intersection(required)
            coverage_ratio    = len(covered) / len(required)
            score += weights["coverage"] * coverage_ratio

            # No-duplicate reward: full credit only when each customer appears exactly once
            if len(all_visited) == len(set(all_visited)):
                score += weights["no_duplicate"]
            else:
                # Partial credit: penalise proportionally to excess visits
                duplicate_ratio = 1.0 - (len(all_visited) - len(set(all_visited))) / max(1, len(all_visited))
                score += weights["no_duplicate"] * max(0.0, duplicate_ratio)
        except Exception:
            pass
...
def optimality_reward_func_cvrp(completions, ground_truth, instance_coords, instance_demands, instance_capacity,
                                **kwargs) -> list[float]:
    """
    Calculate the optimality reward for the CVRP.

    - Fully feasible (feasibility == 1.0): pure optimality score based on gap to optimal.
    - Partially feasible (0 < feasibility < 1.0): scaled partial reward to keep gradient signal.
    - Infeasible (feasibility == 0): no reward.
    """
    scores = []
    ...
    for i, (response, is_feasible) in enumerate(zip(completions, feasible_rewards)):
        # No gradient signal for completely unparseable responses
        if is_feasible == 0.0:
            scores.append(0.0)
            continue

        # Partially feasible: give a small proportional reward to maintain gradient
        if is_feasible < 0.99:
            scores.append(is_feasible * 0.3)
            continue

        # Fully feasible: compute optimality score
        try:
            pred_match = re.search(r"Routes:\s*\[\s*(.*)\]", response, re.DOTALL)
            routes_str = pred_match.group(1).strip()
            predicted_routes = ast.literal_eval(f'[{routes_str}]')
            ...
            distance_matrix = compute_euclidean_distance_matrix(np.array(instance_coords[i]))
            pred_cost = sum(calculate_total_distance(route, distance_matrix) for route in predicted_routes)
            ...
            # Prediction beats or matches optimal → full reward
            if pred_cost <= solution_cost:
                scores.append(1.0)
            else:
                # gap-based score: 1/(1+gap), smooth and bounded in (0, 1)
